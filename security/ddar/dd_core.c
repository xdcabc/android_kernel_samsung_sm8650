...
retry:
	...
	clone = bio_alloc_bioset(orig->bi_bdev, nr_iovecs, orig->bi_opf, GFP_NOIO, ctx->bio_set);
	if (!clone) {
	...
	bio_clone_blkg_association(clone, orig);
...
//				fscrypt_set_ice_dun(req->info->inode, req->u.bio.clone, 0/* temporary */);
		...
		req->u.bio.clone = bio_alloc_clone(bio->bi_bdev, bio, GFP_NOIO, context->bio_set);
		if (!req->u.bio.clone) {
...
static inline struct dd_mmap_control *__get_next_ctr_page(struct dd_transaction *t)
{
	...
	mmap_write_lock(proc->control_vma->vm_mm);
	...
	mmap_write_unlock(proc->control_vma->vm_mm);
		...
		unsigned long int ino, struct dd_transaction *t)
{
	...
	mmap_write_lock(proc->metadata_vma->vm_mm);
	...
	mmap_write_unlock(proc->metadata_vma->vm_mm);
	t->mapped_ino[i] = ino;
	t->num_md_pg++;
}
...
//			MAX_NUM_CONTROL_PAGE - t->num_ctr_pg, require_ctr_pages);
	...
	while (orig->bi_iter.bi_size) {
		...
		if (dir == WRITE) {
			...
			mmap_write_lock(proc->plaintext_vma->vm_mm);
			remap_pfn_range(proc->plaintext_vma,
					p->u.bio.plain_addr, page_to_pfn(plaintext_page), PAGE_SIZE,
					proc->plaintext_vma->vm_page_prot);
			mmap_write_unlock(proc->plaintext_vma->vm_mm);
			mmap_write_lock(proc->ciphertext_vma->vm_mm);
			...
			mmap_write_unlock(proc->ciphertext_vma->vm_mm);
