...
static void set_task_creds_tcnt_test(struct kunit *test)
{
	 ...
	 */
	KUNIT_ASSERT_EQ(test, set_task_creds(main_task, TEST_UID, TEST_FSUID, TEST_EGID,
		TEST_FLAGS), 0);
	KUNIT_ASSERT_EQ(test, set_task_creds(fork_task, TEST_UID, TEST_FSUID, TEST_EGID,
		TEST_FLAGS), 0);
	for (i = 0; i < (DEFEX_MEM_CACHE_SIZE / 2); i++) {
		...
		fork_task->pid += 1;
		fork_task->tgid += 1;
		KUNIT_ASSERT_EQ(test, set_task_creds(main_task, TEST_UID, TEST_FSUID,
			TEST_EGID, TEST_FLAGS), 0);
		KUNIT_ASSERT_EQ(test, set_task_creds(fork_task, TEST_UID, TEST_FSUID,
			TEST_EGID, TEST_FLAGS), 0);
	}
	...
	KUNIT_ASSERT_EQ(test, atomic_read(&mem_cache[CACHE_CRED_DATA].count),
		DEFEX_MEM_CACHE_SIZE);
	KUNIT_ASSERT_EQ(test, atomic_read(&mem_cache[CACHE_HTABLE_ITEM].count),
		DEFEX_MEM_CACHE_SIZE);
	...
	KUNIT_EXPECT_EQ(test, uid, 0U);
	KUNIT_EXPECT_EQ(test, fsuid, 0U);
	KUNIT_EXPECT_EQ(test, egid, 0U);
...
static void set_task_creds_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, set_task_creds(main_task, TEST_UID, TEST_FSUID, TEST_EGID,
		TEST_FLAGS), 0);
	...
	/* T2: Fork task data */
	set_task_creds_tcnt(main_task, 1);
	KUNIT_EXPECT_EQ(test, set_task_creds(fork_task, TEST_UID, TEST_FSUID, TEST_EGID,
		TEST_FLAGS), 0);
	...
	KUNIT_EXPECT_EQ(test, set_task_creds(main_task, TEST_UID + 1, TEST_FSUID + 1,
		TEST_EGID + 1, TEST_FLAGS), 0);

	spin_lock_irqsave(&creds_hash_update_lock, flags);
	query = get_cred_data(TEST_PID_MAIN);
	spin_unlock_irqrestore(&creds_hash_update_lock, flags);
	KUNIT_ASSERT_PTR_NE(test, query, (struct proc_cred_data *)NULL);
	KUNIT_EXPECT_EQ(test, query->cred_flags,
		(unsigned short)(CRED_FLAGS_SUB_UPDATED | CRED_FLAGS_MAIN_UPDATED));
...
static void mem_cache_reclaim_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, atomic_read(&mem_cache[CACHE_CRED_DATA].count),
		DEFEX_MEM_CACHE_SIZE);
...
static void get_task_creds_test(struct kunit *test)
{
	...
	get_task_creds(main_task, &task_uid, &task_fsuid, &task_egid, &task_cred_flags);
	KUNIT_EXPECT_EQ(test, task_uid, 0U);
	KUNIT_EXPECT_EQ(test, task_fsuid, 0U);
	KUNIT_EXPECT_EQ(test, task_egid, 0U);
	...
	KUNIT_ASSERT_EQ(test, set_task_creds(main_task, TEST_UID + 1, TEST_FSUID + 1,
		TEST_EGID + 1, TEST_FLAGS), 0);
	get_task_creds(main_task, &task_uid, &task_fsuid, &task_egid, &task_cred_flags);
	...
	KUNIT_EXPECT_EQ(test, task_cred_flags,
		(unsigned short)(CRED_FLAGS_MAIN_UPDATED | CRED_FLAGS_SUB_UPDATED));
